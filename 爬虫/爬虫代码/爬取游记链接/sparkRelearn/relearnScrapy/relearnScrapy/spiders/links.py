...
def getPage(id):
    ...
    cur1 = conn1.cursor()  # 生成游标对象
    sql = "select record_amount_page from record_amount where city_id=" + str(id)
    cur1.execute(sql)
    results = cur1.fetchall()
    return results[0][0]
...
class BaiduSpider(scrapy.Spider):
    ...
    def parse(self, response, cid, page):
        records = response.body
        # print(records["pageCount"])
        # print(records)
        body = json.loads(records)
        travelInfoList = body['travelInfoList']
        for items in travelInfoList:
            reList = Record_Link()
            reList['city_id'] = cid
            reList['page'] = page
            reList['title'] = items['title']
            reList['link'] = items['jumpUrl']
            # print(reList)
            yield reList
